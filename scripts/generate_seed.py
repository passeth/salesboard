...
# ── 국가 매핑 ──
BUYER_COUNTRY_MAP = {
    "직수출(러시아)": "RU", "직수출(베트남)": "VN", "직수출(베트남2 MYCO)": "VN",
    "직수출(대만)": "TW", "직수출(호주)": "AU", "직수출(캄보디아)": "KH",
    "직수출(라트비아)": "LV", "직수출(슬로바키아)": "SK",
    "직수출(말레이시아)": "MY", "직수출(미얀마)": "MM",
    "직수출(우크라이나)": "UA", "직수출(우크라이나 ICOSMO)": "UA",
    "직수출(우크라이나KBT)": "UA",
    "직수출(위해방정)": "CN", "직수출(유럽)": "EU",
    "직수출(유럽 ICOSMO))": "EU", "직수출(마케도니아)": "MK",
...
def build_organizations(rows):
    ...
    for row in rows:
        ...
        if "스티물" in buyer and row["destination"]:
            stimul_destinations.add(row["destination"])
    ...
    for buyer in sorted(buyer_names):
        ...
        if m:
            ...
            country_id = get_or_create_country(cc, *COUNTRY_NAMES.get(cc, (region, region, "USD")))
...
def main():
    ...
    write_csv(
        ...
        org_list,
        ["id", "parent_org_id", "org_type", "code", "name", "name_en", "country_code", "currency_code", "status"]
    )
    ...
    write_csv(
        OUT_DIR / "02_products.csv",
        prod_list,
        ["id", "sku", "name", "brand", "status"]
    )
    ...
    write_csv(
        ...
        orders,
        ["id", "order_no", "ordering_org_id", "vendor_org_id", "status", "currency_code", "requested_delivery_date", "submitted_at", "confirmed_at", "packing_no_legacy"]
    )
    write_csv(
        OUT_DIR / "04_order_items.csv",
        order_items,
        ["id", "order_id", "line_no", "product_id", "product_code", "requested_qty", "final_qty", "unit_price", "status"]
    )
