...
**FR-DL-01**: 📥 인보이스 다운로드
  - PDF (에바스 로고 + 바이어 정보 + payment terms)
  - **엑셀(xlsx)** — 품목/수량/단가/금액 테이블
  - 이메일 발송 시 PDF + xlsx 첨부

**FR-DL-02**: 📥 패킹리스트 다운로드
  - PDF
  - **엑셀(xlsx)** — 행선지별/팔레트별 시트 분리, 소계 포함

**FR-DL-03**: 🖨️ 쉬핑마크 출력
  - PDF 생성 (A4 1장/팔레트, 큰 글씨 레이아웃)
  - 개별 출력 + **일괄 출력** (전체 팔레트 한번에)
  - 브라우저 프린트 다이얼로그 호출
